...
<div class="sm:flex block items-center justify-between my-[1.5rem] page-header-breadcrumb">
    <div>
        <div class="font-semibold text-[1.125rem] text-defaulttextcolor dark:text-defaulttextcolor/70 !mb-0 ">Role Management</div>
    </div>
    <div>
        @can('Roles create')
                ...
                <tr
                    ...
                    <td>
                        <a class="hs-dropdown-toggle ti-btn ti-btn-sm ti-btn-success !rounded-full"
                            href="{{ route('roles.show', $role->id) }}"><i class="ri-eye-line"></i></a>
                        @if ($role->id > 1)
                            @can('Roles edit')
                                <a class="hs-dropdown-toggle ti-btn ti-btn-sm ti-btn-info !rounded-full"
                                    href="{{ route('roles.edit', $role->id) }}"><i class="ri-edit-line"></i></a>
                            @endcan
                            @can('Roles delete')
